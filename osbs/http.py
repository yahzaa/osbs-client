...
class HttpSession(object):
    ...
    def request(self, *args, **kwargs):
        try:
            stream = HttpStream(*args, verbose=self.verbose, **kwargs)
...
class HttpStream(object):
    ...
    tries to free the resources when it finishes reading the http stream and also when it's gargabe
    collected.
    """
    ...
    def __init__(self, url, method, data=None, kerberos_auth=False,
        ...
        self.finished = False # have read all data?
    ...
    def _select(self):
        ...
        if sel == -1:
            raise OsbsException("CurlMult.select() timed out")
        elif sel == 0:
...
if __name__ == '__main__':
    ...
    r = s.get("https://httpbin.org/stream/99")
    print(r.status_code)
    print(r.headers)
    print(r.content)
